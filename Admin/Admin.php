...
class Admin
{
	...
	public function letter_admin_menu()
	{
		...
		$sub_m[2] = array(
			...
			'capability' => $sub_rules, //capability
			'menu_slug' => $plugin_prefix . 'setiings', //menu slug
...
// функции, реализующие пункты меню! !!!!!!!!!!!!!!!!!
	...
	{
		print "Пока пустая страница. Позже здесь будет какая нибудь статистика или т.п., а сейчас ведется разработка.";
	}
	...
	public function letter_forms(): void
	{
		...
		$wrapper = new AdminWrapper(Admin::$view['current_screen'], ['form_list' => $form_list]);
		$wrapper->construction();
		$wrapper->htmlPrint();
		//print_r(global $current_screen->id);
		//print_r($_POST);
		//print $form_list->formsTableHtml();
	}
	...
	{
		print "Пока что ничего нет. Но не стоит отчаиваться - разработка ведеться!";
	}
